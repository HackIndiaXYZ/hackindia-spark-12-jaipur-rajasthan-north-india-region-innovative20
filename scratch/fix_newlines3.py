...
# Fix bad string literals
content = content.replace('        context = "--- PREVIOUS CONVERSATION CONTEXT ---\\n"\\n"', '        context = "--- PREVIOUS CONVERSATION CONTEXT ---\\n"')
content = content.replace('        context = "--- PREVIOUS CONVERSATION CONTEXT ---\\n"\\n\\n"', '        context = "--- PREVIOUS CONVERSATION CONTEXT ---\\n"')
content = content.replace('context = "--- PREVIOUS CONVERSATION CONTEXT ---\\n"\\n"', 'context = "--- PREVIOUS CONVERSATION CONTEXT ---\\n"')

content = re.sub(r'context = "--- PREVIOUS CONVERSATION CONTEXT ---[^"]*"\n"', 'context = "--- PREVIOUS CONVERSATION CONTEXT ---\\n"', content)
content = re.sub(r'context = "--- PREVIOUS CONVERSATION CONTEXT ---[^"]*"\r?\n"', 'context = "--- PREVIOUS CONVERSATION CONTEXT ---\\n"', content)

# I will just extract the class MemoryManager and replace the whole block again properly.
old_mem = re.search(r'class MemoryManager:.*?(?=class ResearcherAgent:)', content, flags=re.DOTALL)
if old_mem:
...
"""
    content = content.replace(old_mem.group(0), new_memory_manager)
