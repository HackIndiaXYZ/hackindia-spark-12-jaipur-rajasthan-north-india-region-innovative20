...
start_idx = content.find('html_content = """')
if start_idx == -1:
    print("Could not find start")
else:
    end_idx = content.find('"""', start_idx + 18)
    if end_idx == -1:
        print("Could not find end")
    else:
        new_content = content[:start_idx] + f'html_content = """\\n{html}\\n"""' + content[end_idx+3:]
        with open('C:/Users/ayush/.gemini/antigravity/scratch/hackathon_agent/main.py', 'w', encoding='utf-8') as f:
            f.write(new_content)
        print("Redesign applied successfully using precise string slicing.")
